...
Because the underlying data have scatter or associated uncertainties, we need not only best-fit parameter values but also estimates of how uncertain those fitted parameters are; these are sometimes called ``inverse uncertainties'', because they are inferred from the uncertainties in the data.
...
Although our examples focus on transport data, the same framework applies to any temperature-dependent measurements with associated uncertainties, such as reaction rates, mechanical properties, or any data fitted to Arrhenius-type models.
