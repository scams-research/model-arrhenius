...
As a practical illustration, we examine silver-ion conductivity data from molecular dynamics simulations of \ce{AgCrSe2}~\cite{Wang2021}, and compare the Arrhenius and VTF models as candidate descriptions of the temperature dependence.
...
Both models fit the data reasonably well, and the Bayes factor is $\ln{(B_{\beta\alpha})}=\;$\variable{output/bayes_40.txt}, where the uncertainty is a \qty{95}{\percent} confidence interval over nested sampling repeats. 
...
To investigate how the Bayes factor depends on data quality, we varied the fitting window length from \qty{40}{\pico\second} to \qty{140}{\pico\second} (\cref{fig:agcrse2}e).
The start of the fitting window, $\Delta t_\mathrm{min}$, is temperature-dependent (\cref{tab:lengths}), so for a given fitting window length, the total trajectory length analysed varies with temperature.
At each temperature, MSDs were computed at lag times from $\Delta t_\mathrm{min}$ to $\Delta t_\mathrm{min}$ plus the fitting window length, in increments of $\Delta(\Delta t)$ (\cref{tab:lengths}).
