...
We quantify this using the likelihood: the probability of observing our data given a particular set of parameter values. This is a conditional probability, written $\pr{\bm{x}}{\mathbf{\theta}}$, where $\bm{x}$ denotes our data and $\mathbf{\theta}$ the parameters. 
...
This is the posterior distribution, written $\pr{\mathbf{\theta}}{\bm{x}}$.
Likelihood and posterior are related but not the same: $\pr{\bm{x}}{\mathbf{\theta}}$ is not the same as $\pr{\mathbf{\theta}}{\bm{x}}$. 
The relationship between them is given by Bayes' theorem:
\begin{equation}
\pr{\mathbf{\theta}}{\bm{x}} = \frac{\pr{\bm{x}}{\mathbf{\theta}} \times p(\mathbf{\theta})}{p(\bm{x})}.
\end{equation}
Here, $\pr{\mathbf{\theta}}{\bm{x}}$ is the posterior: the probability of parameters $\mathbf{\theta}$ given the data. 
$\pr{\bm{x}}{\mathbf{\theta}}$ is the likelihood. 
$p(\mathbf{\theta})$ is the prior: our knowledge about plausible parameter values before seeing the data. 
...
\begin{equation}
\pr{\mathbf{\theta}}{\bm{x}} \propto \pr{\bm{x}}{\mathbf{\theta}} \times p(\mathbf{\theta}).
\end{equation}
This means we can characterise the posterior by evaluating the likelihood and prior across parameter space.

The prior distribution, $p(\mathbf{\theta})$, encodes what we know about plausible parameter values before examining the data. 
...
% \begin{widetext}
    \begin{equation}
        \pr{\bm{x}}{\mathbf{\theta}} = (2\pi)^{-\frac{k}{2}}|\bm{\Sigma}|^{-\frac{1}{2}}\exp\left\{-\frac{1}{2}\left[\bm{\mu} - \bm{x}\right]^\top\bm{\Sigma}^{-1}\left[\bm{\mu} - \bm{x}\right]\right\},
        \label{eqn:gen}
    \end{equation}
% \end{widetext}
%
where $k$ is the number of measured temperatures, $\bm{\mu}$ is a vector of the measured conductivity values, $\Sigma$ is the covariance matrix of measurement uncertainties, and $\pr{\bm{x}}{\mathbf{\theta}}$ is the model prediction for parameters $\mathbf{\theta}$.
...
The activation energy is approximately normally distributed, with mean and \qty{95}{\percent} credible interval of \input{output/activation_energy_llzo.txt}. 
The pre-exponential factor, however, is not normally distributed---it follows an approximately log-normal distribution. 
For this parameter, we report the median and \qty{95}{\percent} credible interval: \input{output/preexp_llzo.txt}.
...
If we denote the parameters of model $m$ as $\mathbf{\theta}_m$, this is:
\begin{equation}
\pr{\bm{x}}{m} = \idotsint_{\mathbf{\theta}_m} \pr{\bm{x}}{\mathbf{\theta}_m, m} \,\, \pr{\mathbf{\theta}_m}{m}\,\mathrm{d}\mathbf{\theta}_m.
\end{equation}
...
If $B > 1$, the data favour model $\beta$; if $B_{\beta\alpha} < 1$, they favour model $\alpha$. 
The magnitude indicates the strength of the evidence. 
A common interpretation, due to Kass and Raftery, is that $\ln(B) > 5$ (corresponding to $B > 150$) constitutes ``very strong'' evidence for the preferred model~\cite{Kass1995}.
...
Both models fit the data reasonably well, and the Bayes factor is $\ln{(B_{\beta\alpha})}=\;$\input{output/bayes_40.txt}, where the uncertainty is a \qty{95}{\percent} confidence interval. 
...
The Bayes factor increases to $\ln(B_{\beta\alpha}) =\,\input{output/bayes_140.txt}$, well above the threshold of \num{5}, and strong evidence for the more complex VTF model over the simpler Arrhenius alternative.
...
This distribution is broad and asymmetric, with a median and \qty{95}{\percent} credible interval of \input{output/d_300.txt}.
...
\begin{table}
    ...
    \begin{tabular}{ccc}
        ...
        \midrule
        300 & 70.0 & \input{output/dt300.txt} \\
        350 & 50.0 & \input{output/dt350.txt} \\
        400 & 30.0 &\input{output/dt400.txt} \\
        500 & 25.0 & \input{output/dt500.txt} \\
        600 & 22.5 & \input{output/dt600.txt} \\
        700 & 20.0 & \input{output/dt700.txt} \\
        \bottomrule
    \end{tabular}
\end{table}
