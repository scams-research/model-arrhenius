...
As an example of this procedure, we analyse lithium-ion conductivity data from molecular dynamics simulations of the cubic phase of lithium lanthanum zirconium oxide (c-LLZO), a solid electrolyte of interest for solid-state batteries (see Methods for simulation details).
At each of four temperatures (\qtylist{500; 600; 700; 800}{\kelvin}), we used approximate Bayesian regression of the MSD against time~\cite{McCluskey2024a} to obtain a conductivity estimate with associated uncertainty.
...
Self-diffusion coefficients were converted to illustrative conductivity estimates via the Nernst-Einstein relation, assuming a Haven ratio $H_\mathrm{R} = 1$.
A temperature-independent Haven ratio would uniformly scale the conductivity values without affecting the temperature dependence or the analysis.
A temperature-dependent Haven ratio would introduce additional curvature, potentially shifting the model comparison towards the VTF model, but the analysis procedure would be unchanged.
...
The onset of diffusive behaviour was identified by visual inspection of log-log MSD plots, selecting the region where the MSD is linear in time.
