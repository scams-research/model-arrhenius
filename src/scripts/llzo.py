...
gs = gridspec.GridSpec(3, 2, figure=fig, wspace=0.2, hspace=0.5, height_ratios=[1, 0.8, 0.6])
...
axes[-1].set_ylabel(r"$\sigma_{\mathrm{Li}^+}T$ / $10^5$ mS cm$^{-1}$ K")
...
ax2.set_xlabel("$T$ / K", labelpad=4)
ax2.set_xticks(temp)
labels.append(f"({ALPHABET[0]})")
...
axes[-1].set_xlabel(r'$E_{\mathrm{a}}\,|\,\sigma_{\mathrm{Li}^+}T$ / eV')
axes[-1].set_yticks([])
# axes[-1].set_aspect('equal')
labels.append(f"({ALPHABET[2]})")
...
axes[-1].set_xlabel(r"$A\,|\,\sigma_{\mathrm{Li}^+}T$ / $10^6$ mS cm$^{-1}$ K")
axes[-1].set_xticks([2, 3, 4])
# axes[-1].set_aspect('equal')
axes[-1].set_yticks([])
labels.append(f"({ALPHABET[3]})")
...
axes[-1].text(-0.23, 0.5, r"$A\,|\,\sigma_{\mathrm{Li}^+}T$ / $10^6$ mS cm$^{-1}$ K", va='center', rotation=90, transform=axes[-1].transAxes)
axes[-1].set_xticks([0.12, 0.14])
axes[-1].set_yticks([2, 3])
axes[-1].set_xlabel(r'$E_{\mathrm{a}}\,|\,\sigma_{\mathrm{Li}^+}T$ / eV')
labels.append(f"({ALPHABET[1]})")
...
for i, ax in enumerate(axes):
    if i == 3:  # contour plot (b) — use axes[0] x position so it aligns with (a)
        x = axes[0].get_position().x0
        y = ax.get_position().y1
        fig.text(x, y, labels[i], ha='left', va='bottom')
    else:
